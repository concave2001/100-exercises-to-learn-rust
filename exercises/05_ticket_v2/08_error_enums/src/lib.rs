...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match t {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleError {e: e}) => {
            panic!("{}", e);
        },
        Err(TicketNewError::DescriptionError {e: _}) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { e: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { e: "Title cannot be longer than 50 bytes".to_string() });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { e: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { e: "Description cannot be longer than 500 bytes".to_string() });
        }
